...
## [2026-06-21] Real-data CSV swap + finalization (Step 7)
- What I did: Added module-level CSV override state to `profiles.py` via `load_csv_profiles(solar_csv, load_csv)` and `reset_csv_profiles()` — existing function signatures unchanged. Generated 96-row CSV files (`backend/data/solar_day.csv`, `backend/data/load_day.csv`) with a realistic cloud dip and morning/evening load peaks. Added `profiles.source: csv` to `config.yaml`; `runner.py` calls `_configure_profiles(cfg)` at `build_sim` time so the swap is automatic. Added 7 new CSV-path tests (total: 42 passed). Ran all existing gates unchanged with CSV active (check_market + check_ws both green). Wrote `backend/main.py` (single-command entrypoint: `python backend/main.py`; auto-starts Mosquitto via brew services if not running) and `Makefile` (`make run`, `make test`, `make check-all`).
- Files touched (backend/ only): `backend/sim/physics/profiles.py`, `backend/sim/runner.py`, `backend/data/solar_day.csv`, `backend/data/load_day.csv`, `backend/config.yaml`, `backend/main.py`, `backend/tests/test_profiles.py`, `Makefile`.
- Decisions/assumptions (+ defaults): CSV state is module-level (not class-level) because `solar.py`/`load.py` import the functions at import time; no agent changes needed. `reset_csv_profiles()` is exposed for tests. CSV format is `day_phase, output_fraction|load_fraction` (normalized 0–1), scaled by inverter_kw / (peak_kw - base_kw + base_kw) in the function, so all config knobs still apply. `_REPO_ROOT` in runner.py is derived from `__file__` so the relative CSV paths in config work regardless of CWD.
- Open questions / risks for the human: none. Backend is complete — all 7 steps done, all gates green.
- Next step: Codex completes frontend (Steps 9–15 in the frontend plan).
